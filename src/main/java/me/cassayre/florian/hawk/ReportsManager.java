 ...
 * </pre>
 */
public class ReportsManager extends QuartzComponent {
     ...
     * changed.
     */
    private final Map<UUID, String> lastBackupDigest = new HashMap<>();
    ...
    public static void init(final JavaPlugin plugin) {
        ...
        if (Hawk.get() == null || Hawk.get().getManager() == null) {
            instance = QuartzLib.loadComponent(ReportsManager.class);
        }
    }
    ...
    protected void onEnable() {
        QuartzLib.registerEvents(new PlayerConnectionListener(this));
        QuartzLib.registerEvents(new PlayerDamagesListener(this));
    ...
    private void setUserAgent() {
        ...
        try {
            ...
            minecraftVersion =
                    (String) Reflection.call(Reflection.getFieldValue(Bukkit.getServer(), "console"), "getVersion");
        }
     ...
     */
    public Stream<Report> getTrackedReportsFor(final OfflinePlayer player,
                                               boolean includeReportsWerePlayerIsNotTracked) {
     ...
     */
    public void backup(final Report report, final Callback<File> callbackSuccess,
                       final Callback<Throwable> callbackError) {
        report.getPlayers().stream().filter(player -> report.isTracked(player.getUniqueId()))
                .forEach(ReportPlayer::collectStatistics);
        ...
        ReportsWorker.save(
                ...
                new File(saveDirectory,
                        "backups/" + dateSlug(report.getStartDate()) + "-" + slug(report.getTitle()) + "-backup-" +
                                dateSlug(System.currentTimeMillis()) + ".json"),
     ...
     */
    public void save(final Report report, final Callback<File> callbackSuccess,
                     final Callback<Throwable> callbackError) {
        save(
                report,
     ...
     */
    public void save(final Report report, File location, final Callback<File> callbackSuccess,
                     final Callback<Throwable> callbackError) {
        report.getPlayers().stream().filter(player -> report.isTracked(player.getUniqueId()))
                .forEach(ReportPlayer::collectStatistics);
     ...
     *                        returned.
     */
    public void publish(final Report report, final Callback<URI> callbackSuccess,
                        final Callback<Throwable> callbackError) {
        report.getPlayers().stream().filter(player -> report.isTracked(player.getUniqueId()))
                .forEach(ReportPlayer::collectStatistics);
